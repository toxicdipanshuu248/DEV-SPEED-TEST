...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`⚡ DEV SPEED MATRIX server is running: http://0.0.0.0:${PORT}`);
  // Startup check: is index.html in place?
  const ok = ['public/index.html', 'index.html'].some(p => fs.existsSync(path.join(__dirname, p)));
  if (ok) {
    console.log('✅ index.html found — ready to roll!');
  } else {
    console.warn('⚠️  WARNING: index.html not found! Make sure public/index.html is uploaded to the repo.');
  }
});
